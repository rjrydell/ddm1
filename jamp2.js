	...
	**/
	...
	function ampExtension(options)
	{
        ...
        */
		...
		var headers = ['block','trial','prime_cat', 'prime_word', 'target_cat', 'target_word', 'resp', 'rt'];
                var myLogs = [];
                var iLog;
		...
		// Split the participant's image pool once so each face appears only once.
		var allFaceMedia = _.shuffle(piCurrent.targetCats[0].mediaArray.slice());
		if (allFaceMedia.length < 255)
		{
			throw new Error('Need at least 255 unique face images.');
		}
		var practiceFaces = allFaceMedia.slice(0, 5);
		var mainFaces = allFaceMedia.slice(5, 255);

		// Keep practice separate from the real task.
		piCurrent.exampleTargetStimulus.sameAsTargets = false;
		piCurrent.exampleTargetStimulus.mediaArray = practiceFaces;
		piCurrent.targetCats[0].mediaArray = mainFaces;

		var basicTrial = piCurrent.responses == 2 ? 'basicSort' : 'basicRate';

		function makeTargetStim(mediaObj, aliasText)
		{
			return {
				inherit:'Default',
				data : {handle:'targetStim', alias:aliasText},
				css: piCurrent.targetStimulusCSS,
				media: mediaObj
			};
		}
		...
		{
			var primeStim = opts.example ?
				{inherit:{set:'exPrimeStim', type:'exRandom'}} :
				{inherit:opts.primeName};

			return {
				inherit : {set:basicTrial},
				data : {
					fixationDuration: opts.fixationDuration,
					primeDuration: opts.primeDuration,
					postPrimeDuration: opts.postPrimeDuration,
					targetDuration: opts.targetDuration,
					ITI: opts.ITI,
					condition: opts.condition,
					useFixation : opts.useFixation,
					block: opts.block
				},
				stimuli : [
					{inherit:'rate1'}, {inherit:'rate2'}, {inherit:'rate3'},
					{inherit:'rate4'}, {inherit:'rate5'}, {inherit:'rate6'}, {inherit:'rate7'},
					{inherit: opts.example ? 'exFixationStim' : 'fixationStim'},
					{inherit: opts.example ? 'exMaskStim' : 'maskStim'},
					primeStim,
					makeTargetStim(opts.targetMedia, opts.targetAlias)
				]
			};
		}

		function shuffledPrimeOrder(countA, countB)
		{
			var order = [];
			var i;
			for (i = 0; i < countA; i++) { order.push(piCurrent.primeCats[0].nameForLogging); }
			for (i = 0; i < countB; i++) { order.push(piCurrent.primeCats[1].nameForLogging); }
			return _.shuffle(order);
		}

		var practicePrimeOrder = shuffledPrimeOrder(3, 2);
		var mainPrimeOrder = shuffledPrimeOrder(125, 125);
		...
		***/
		...
		if (piCurrent.trialsInExample > 0)
		{
			...
			var examplePrimeOrder = shuffledPrimeOrder(3, 2);
			//Instructions trial
			trialSequence.push(
				{
					inherit : 'inst', 
			...
			for (var e = 0; e < practiceFaces.length; e++)
			{
				trialSequence.push(buildTrial({
					example: true,
					block: blockNum,
					condition: 'example',
					primeName: examplePrimeOrder[e],
					targetMedia: practiceFaces[e],
					targetAlias: 'practice_' + (e + 1),
		...
		for (iBlock = 0; iBlock < piCurrent.trialsInBlock.length; iBlock++)
		{
			...
			//trials
			for (var t = 0; t < piCurrent.trialsInBlock[iBlock]; t++)
			{
				primeName = mainPrimeOrder[mainTrialIndex];
				targetName = 'face_' + (mainTrialIndex + 1);
				trialSequence.push(buildTrial({
					example: false,
					block: blockNum,
					condition: primeName,
					primeName: primeName,
					targetMedia: mainFaces[mainTrialIndex],
